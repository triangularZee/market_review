...
Daily Review 첫 bullet은 가능하면 <intraday_index_facts:국가>를 사용해 "상승/하락 출발 → 장중 고점·저점 → 종가 회복/반락" 순서로 장중 궤적을 수치와 함께 서술하세요.
저점 대비 종가 회복률이 유의미하면 "장중 저점에서 반등"이라고 쓸 수 있지만, 분봉 데이터가 없으므로 "오전장", "오후 개장 직후", "V자 반등"처럼 정확한 발생 시각·순서를 단정하지 마세요.
시장 전체 거래대금과 과거 비교 원자료가 있을 때만 "거래대금 감소", "최근 N개월 최저"를 쓰세요. 거래대금 상위 종목 합계를 시장 전체 거래대금으로 표현하지 마세요.
...
8. 같은 국가의 당일 뉴스 중 Reuters, Bloomberg, Financial Times, Wall Street Journal, CNBC와 해당국 주요 경제지 기사를 우선 사용하세요. 검색 결과 순서를 그대로 따르지 말고 raw data와 가장 잘 맞는 촉매를 고르세요.
9. 복수의 독립 매체가 같은 촉매를 다루면 핵심 뉴스 플로우로 간주해 <한 줄 평>과 Daily Review에 반영하세요. 한 매체만 언급한 사안은 확인된 사실 범위를 넘겨 시장 전체 원인으로 확대하지 마세요.
10. <한 줄 평> 첫 문장은 지수·주도/부진 섹터와 핵심 뉴스 촉매를 결합하고, 두 번째 문장은 거래대금 상위 종목의 확산 또는 쏠림을 설명하세요.
11. Daily Review는 가격만 재진술하지 마세요. 관련 뉴스가 있으면 1개 bullet에서 "뉴스 촉매 → 해당 섹터·종목 반응 → 지수 영향" 순서로 연결하고, 나머지 bullet은 raw data의 반대편 흐름이나 시장 내부 차별화를 설명하세요.
12. 뉴스 제목에 없는 세부 수치·발언·정책 내용을 만들어내지 말고, 리포트 본문에 매체 이름이나 출처 목록을 나열하지 마세요.
13. <intraday_index_facts:국가>가 있으면 시가 갭, 장중 고가·저가, 종가의 저점 대비 회복 또는 고점 대비 반락을 우선 활용해 장세의 실제 진행을 설명하세요. OHLC만으로 고가와 저가가 발생한 정확한 시간 순서는 추정하지 마세요.
